...
fn show_key() {
    println!("DEBUG: Current dir: {:?}", env::current_dir());
    println!("DEBUG: Checking for PRIVATE_KEY...");
    match env::var("PRIVATE_KEY") {
        Ok(key) => println!("PRIVATE_KEY: {}", key),
        Err(e) => {
            println!("PRIVATE_KEY not found in .env");
            println!("Error: {:?}", e);
            println!("\nTip: Make sure .env file exists in the current directory");
        }
    }
}
...
fn send_fragments(mode: &str) {
    ...
    // Load receiver public key from env
    let receiver_pk_hex = env::var("CRYPT_RECIEVER_PUBLIC_KEY").expect("CRYPT_RECIEVER_PUBLIC_KEY not found");
    let _receiver_pk_bytes = hex::decode(receiver_pk_hex).expect("Invalid receiver public key");
    
    // For pqc_kyber, PublicKey is [u8; KYBER_PUBLICKEYBYTES]
    // Generate a keypair and save it to a file for reconstruction
    ...
    let keypair = if std::path::Path::new(keypair_file).exists() {
        ...
        // Deserialize keypair (public + secret keys)
...
fn send_email(c: &str, e: &str, ciphertext: &str, id: &str, seq: &str) -> Result<(), String> {
    ...
    // Build email body with custom headers in text format
    let body = format!(
...
fn reconstruct_key() {
    println!("Starting key reconstruction from emails...");
    ...
    let receiver_sk_hex = env::var("CRYPT_RECIEVER_PRIVATE_KEY").expect("CRYPT_RECIEVER_PRIVATE_KEY not found");
    let _receiver_sk_bytes = hex::decode(receiver_sk_hex).expect("Invalid receiver private key");
    
    // Generate keypair (load from file if exists)
    let mut rng = rand::thread_rng();
    let keypair_file = ".rust_keypair.bin";
    let keypair = if std::path::Path::new(keypair_file).exists() {
    ...
    let client = match imap::connect((imap_host.as_str(), imap_port.parse::<u16>().unwrap()), &imap_host, &tls) {
        Ok(c) => {
            println!("Connected to IMAP server");
            c
        }
        Err(e) => {
            println!("Failed to connect to IMAP server: {}", e);
            return;
        }
    };

    println!("Logging in as: {}", imap_user);
    let mut imap_session = match client.login(&imap_user, &imap_pass) {
        Ok(s) => {
            println!("Login successful");
            s
        }
        Err(e) => {
            println!("Failed to login to IMAP: {:?}", e);
            return;
        }
    };
    ...
    // Search for all emails with subject "MAIL PRIVATE KEY"
    ...
    if messages.is_empty() {
        println!("No messages found with subject 'MAIL PRIVATE KEY'!");
    ...
    println!("Processing last 5 messages: {:?}", recent_messages);
    ...
    for msg_id in recent_messages.iter() {
        println!("Fetching message ID: {}", msg_id);
        let messages = imap_session
        ...
        for message in messages.iter() {
            if let Some(body) = message.body() {
                ...
                // Get email body text and parse custom headers from it
                let body_text = parsed.get_body().unwrap_or_default();
                println!("Body text preview: {}", &body_text.chars().take(200).collect::<String>());
                ...
                println!("Extracted from body - ID: {}, Seq: {}, C len: {}, E len: {}, CT len: {}", 
                    id, seq, c.len(), e.len(), ciphertext.len());

                if id.is_empty() {
                    println!("Skipping message - no ID found");
                    continue;
                }
                ...
                println!("Processing message with ID: {}, seq: {}", id, seq);

                // Unlock message
                let c_bytes = match hex::decode(&c) {
                    Ok(b) => b,
                    Err(e) => {
                        println!("Invalid C hex: {}", e);
                        continue;
                    }
                };
                let e_bytes = match hex::decode(&e) {
                    Ok(b) => b,
                    Err(e) => {
                        println!("Invalid E hex: {}", e);
                        continue;
                    }
                };
                let ct_bytes = match hex::decode(&ciphertext) {
                    Ok(b) => b,
                    Err(e) => {
                        println!("Invalid ciphertext hex: {}", e);
                        continue;
                    }
                };
...
mod tests {
    ...
    fn test_time_lock() {
        ...
        let t = 1000; // Small T for testing
        
        let a_str = "";
        let n_str = "";
        
        let a = a_str.parse::<num_bigint::BigUint>().unwrap();
        let n = n_str.parse::<num_bigint::BigUint>().unwrap();
        ...
        // Lock message
        let lock_result = lock_message(message, t, &n, &a, &k, &keypair.public).unwrap();
        println!("Ciphertext size: {}", lock_result.ciphertext.len());
        
        // Unlock message
        let recovered = unlock_message(
            &lock_result.c,
            &lock_result.e,
            &a,
            &n,
            t,
            &lock_result.ciphertext,
            &keypair.secret,
        ).unwrap();
        
        assert_eq!(message, recovered.as_slice());
        println!("✓ Time-lock test passed!");
    }
}
