...
fn ensure_fah_epochs(domain: &str, fah: usize) {
    ...
    if needed > 0 {
        log_domain(domain, &format!(
            "FAH: Need {} more epoch(s) to maintain horizon of {}",
            needed, fah));
        ...
        for i in 0..needed {
            ...
            if let Some(&latest) = new_epochs.last() {
                ...
                if !file_exists(&format!("{}/eka.txt", ef)) {
                    ...
                    let out = run_kda(&["eka", domain, &latest_str]);
                    print!("{}", out);
                }
...
fn run_domain(domain: String, epoch_interval: u64, num_fragments: usize, fah: usize, running: Arc<AtomicBool>) {
    ...
    // Step 1: Ensure FAH epochs exist on startup
    ensure_fah_epochs(&domain, fah);

    while running.load(Ordering::SeqCst) {
        // Find the oldest undisclosed epoch to process
        ...
        if !file_exists(&format!("{}/kdr.txt", epoch_folder)) {
            ...
            let out = run_kda(&["kdr", &domain, &epoch_id_str]);
            print!("  {}", out);
            continue;
        }
        ...
        if fdr_count < num_fragments {
            ...
            let out = run_kda(&["fragment", &domain, &epoch_id_str]);
            print!("  {}", out);
        ...
        if !file_exists(&format!("{}/dpr.txt", epoch_folder)) {
            ...
            let out = run_kda(&["dpr", &domain, &epoch_id_str]);
            print!("  {}", out);
            // Epoch just completed - maintain FAH
            log(&domain, &epoch_id_str, "Epoch disclosure COMPLETE.");
        ...
        // Already fully disclosed - maintain FAH and move on
        ensure_fah_epochs(&domain, fah);
...
fn main() {
    ...
    log_global("=== KDA Service Started ===");
